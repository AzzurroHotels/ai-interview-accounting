...
const NOTIFY_FUNCTION_CANDIDATES = ["notify-careers"];
...
const QUESTIONS = [
  {
    id: "q1-bank-reconciliation",
    text: "How do you ensure accuracy when reconciling bank statements?",
    followups: [],
  },
  {
    id: "q2-discrepancy",
    text: "If you find a discrepancy in the books, what steps do you take to resolve it?",
    followups: [],
  },
  {
    id: "q3-tax-regulations",
    text: "How do you stay updated with tax regulations or accounting standards?",
    followups: [],
  },
  {
    id: "q4-prioritization",
...
const els = {
  ...
  // speed test
  stDlValue: document.getElementById("st-dl-value"),
  stUlValue: document.getElementById("st-ul-value"),
  stLatValue: document.getElementById("st-lat-value"),
  stStatusText: document.getElementById("st-status-text"),
  stSpinner: document.getElementById("st-spinner"),
  stVerdict: document.getElementById("st-verdict"),
  stDownloadStat: document.getElementById("st-download"),
  stUploadStat: document.getElementById("st-upload"),
  stLatencyStat: document.getElementById("st-latency"),
  speedtestRetryBtn: document.getElementById("speedtestRetryBtn"),
  speedtestContinueBtn: document.getElementById("speedtestContinueBtn"),
...
let speedTestResults = { download_mbps: null, upload_mbps: null, latency_ms: null };
...
function mbps(bytes, ms) {
  return ((bytes * 8) / (ms / 1000) / 1_000_000).toFixed(1);
}
...
function speedColor(mbpsVal) {
  const n = parseFloat(mbpsVal);
  if (n >= 5) return "good";
  if (n >= 1.5) return "warn";
  return "bad";
}

function setStatResult(statEl, valueEl, value, colorClass) {
  valueEl.textContent = value;
  statEl.classList.remove("good", "warn", "bad");
  statEl.classList.add(colorClass);
}
...
async function measureDownload() {
  const bytes = 3_000_000; // 3 MB
  const url = `https://speed.cloudflare.com/__down?bytes=${bytes}`;
  const t0 = performance.now();
  const res = await fetch(url, { cache: "no-store" });
  await res.arrayBuffer();
  const ms = performance.now() - t0;
  return { mbpsVal: mbps(bytes, ms), ms };
}

async function measureUpload() {
  const bytes = 2_000_000; // 2 MB
  const body = new Uint8Array(bytes);
  crypto.getRandomValues(body.subarray(0, Math.min(65536, bytes))); // fill first 64KB randomly
  const t0 = performance.now();
...
async function runSpeedTest() {
  // Reset UI
  els.stDlValue.textContent = "—";
  els.stUlValue.textContent = "—";
  els.stLatValue.textContent = "—";
  els.stDownloadStat.className = "speedstat";
  els.stUploadStat.className = "speedstat";
  els.stLatencyStat.className = "speedstat";
  els.stVerdict.className = "speedtest-verdict hidden";
  els.stVerdict.textContent = "";
  els.speedtestContinueBtn.disabled = true;
  els.speedtestRetryBtn.disabled = true;
  els.stSpinner.style.display = "inline-block";

  let dlMbps = null, ulMbps = null, latMs = null;

  try {
    // 1. Latency
    els.stStatusText.textContent = "Measuring latency…";
    latMs = await measureLatency();
    setStatResult(els.stLatencyStat, els.stLatValue, `${latMs} ms`, latencyColor(latMs));

    // 2. Download
    els.stStatusText.textContent = "Testing download speed…";
    const dl = await measureDownload();
    dlMbps = dl.mbpsVal;
    setStatResult(els.stDownloadStat, els.stDlValue, `${dlMbps} Mbps`, speedColor(dlMbps));

    // 3. Upload
    els.stStatusText.textContent = "Testing upload speed…";
    const ul = await measureUpload();
    ulMbps = ul.mbpsVal;
    setStatResult(els.stUploadStat, els.stUlValue, `${ulMbps} Mbps`, speedColor(ulMbps));

    // Save results for DB submission
    speedTestResults = {
      download_mbps: parseFloat(dlMbps),
      upload_mbps: parseFloat(ulMbps),
      latency_ms: latMs,
    };

    // 4. Verdict
    els.stSpinner.style.display = "none";
    els.stStatusText.textContent = "Test complete.";

    const ulNum = parseFloat(ulMbps);
    if (ulNum < 1.5) {
      els.stVerdict.className = "speedtest-verdict bad";
      els.stVerdict.textContent =
        "⚠️ Your upload speed is low (" + ulMbps + " Mbps). Recordings may take a while to submit. " +
        "Try moving closer to your router or switching to a wired connection if possible.";
    } else if (ulNum < 5) {
      els.stVerdict.className = "speedtest-verdict warn";
      els.stVerdict.textContent =
        "✔ Connection looks adequate. Upload may be slightly slow — please keep this tab open during submission.";
    } else {
      els.stVerdict.className = "speedtest-verdict good";
      els.stVerdict.textContent = "✔ Great connection! Your recordings should upload quickly.";
    }

  } catch (e) {
    console.warn("Speed test error:", e);
    els.stSpinner.style.display = "none";
    els.stStatusText.textContent = "Test could not complete.";
    els.stVerdict.className = "speedtest-verdict warn";
    els.stVerdict.textContent =
      "Could not measure connection speed (network or CORS issue). You can still proceed — just ensure you have a stable internet connection.";
  }

  els.speedtestContinueBtn.disabled = false;
  els.speedtestRetryBtn.disabled = false;
}

els.speedtestRetryBtn.addEventListener("click", () => {
  runSpeedTest();
});

els.speedtestContinueBtn.addEventListener("click", () => {
  showStep(els.practice);
  speak("Let’s do a quick practice. This is not scored. Please say your name and today’s date, then give a short professional introduction like you’re starting an accountant interview (your experience, specialisation, and a recent achievement you are proud of).");
});
...
function loadQuestion() {
  ...
  // AI voice reads the question (accountant interview)
...
async function submitInterview() {
  ...
  const { data: interview, error: interviewErr } = await supabase
    ...
    .insert({
      ...
      download_mbps: speedTestResults.download_mbps,
      upload_mbps: speedTestResults.upload_mbps,
      latency_ms: speedTestResults.latency_ms,
    })
    .select()
    .single();
