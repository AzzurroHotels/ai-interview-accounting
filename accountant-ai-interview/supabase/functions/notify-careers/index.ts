...
serve(async (req) => {
  ...
  try {
    ...
    // Helper: colour-code upload speed
    function uploadSpeedColor(mbps: number | null): string {
      if (mbps == null) return "#333";
      if (mbps >= 5) return "#1a6b40";
      if (mbps >= 1.5) return "#7a5500";
      return "#8b1a1a";
    }

    function fmtSpeed(val: number | null, unit: string): string {
      return val != null ? `${val} ${unit}` : "Not recorded";
    }
    ...
    const emailHtml = `
      <div style="font-family:sans-serif; max-width:640px; margin:0 auto; color:#333;">
        <div style="background:#0066cc; padding:24px; border-radius:8px 8px 0 0;">
          <h1 style="margin:0; color:white; font-size:22px;">New Accounting Interview Submission</h1>
        ...
        <div style="padding:24px; background:white; border:1px solid #e0e0e0;">
          ...
          <table style="width:100%; border-collapse:collapse; margin-bottom:24px;">
            ...
            <tr><td style="padding:8px; color:#666; width:140px;">Tab Switches</td><td style="padding:8px;">${interview.visibility_hidden_count}</td></tr>
            ...
            <tr><td style="padding:8px; color:#666;">Download Speed</td><td style="padding:8px;">${fmtSpeed(interview.download_mbps, "Mbps")}</td></tr>
            <tr style="background:#f9f9f9;"><td style="padding:8px; color:#666;">Upload Speed</td><td style="padding:8px; font-weight:bold; color:${uploadSpeedColor(interview.upload_mbps)};">${fmtSpeed(interview.upload_mbps, "Mbps")}</td></tr>
            <tr><td style="padding:8px; color:#666;">Latency</td><td style="padding:8px;">${fmtSpeed(interview.latency_ms, "ms")}</td></tr>
          </table>
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `New Accounting Interview Submission: ${interview.candidate_name}`,
        html: emailHtml,
      }),
    });
